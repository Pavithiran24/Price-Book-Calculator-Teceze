...
export const PriceSummary: React.FC<PriceSummaryProps> = ({ calculation, onReset, language = 'en', currency = 'USD' }) => {
  ...
  const generatePDF = async () => {
    ...
    // Add logo (PNG) - center it
    try {
      const logoUrl = '/teceze.png';
    ...
    doc.text('Total:', col1, yPosition, { align: 'left' });
    doc.text(formatCurrency(calculation.total), col3, yPosition, { align: 'right' });
    doc.setTextColor(0);
    ...
    doc.setFontSize(12);
    doc.setTextColor(40, 70, 200);
    doc.text('Notes:', col1, yPosition, { align: 'left' });
    doc.setTextColor(0);
    doc.setFontSize(11);
    doc.text(customNote, col1, yPosition + 8, { maxWidth: pageWidth - 50 });
